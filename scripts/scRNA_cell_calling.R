...
if (expect >0) {
    ...
    o <- order(br.out$rank)
    cutoff = nrow(subset(bc,bc$UB>=br.out$inflection))
    m = br.out$inflection
}else {
    expect = force
    cutoff = expect
...
write.table(cc_1W$BARCODE, file=paste(opt$output,"/cell_barcodes_10K.txt",sep=""),row.names=FALSE,col.names=FALSE,quote=FALSE)
